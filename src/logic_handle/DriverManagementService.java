...
public class DriverManagementService {
    ...
    public static void createDrivingSchedule() {
        ...
        for (int i = 0; i < newDriverNumber; i++) {
            ...
            MainRun.DRIVER_MANS.add(driverManagement);
        }

        FileUtil.writeDataToFile(MainRun.DRIVER_MANS, DRIVER_MANAGEMENT_FILE_NAME);
    }
    ...
    public static void showData() {
        MainRun.DRIVER_MANS.forEach(System.out::println);
    }

    public static void sortByDriverName() {
        MainRun.DRIVER_MANS.sort(Comparator.comparing(o -> o.getDriver().getName()));
        showData();
    }

    public static void sortByNumberOfBusLine() {
        MainRun.DRIVER_MANS.sort(Comparator.comparing(o -> o.getBusLines().size()));
        showData();
    }
    ...
    public static void calculateTotalRound() {
        MainRun.DRIVER_MANS.forEach(driverManagement -> { driverManagement.getTotalRound(); });
        System.out.println("Bảng thống kê khoảng cách chạy xe trong ngày của mỗi lái xe là: ");

        MainRun.DRIVER_MANS.forEach(driverManagement -> {
